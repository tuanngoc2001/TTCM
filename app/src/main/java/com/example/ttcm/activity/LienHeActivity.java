...
//        }
    ...
    public void sentMess(View view) {
        //Trình quản lý cho phép gửi tin nhắn đến số điện thoại bất kỳ
        SmsManager smsManager = SmsManager.getDefault();

        Intent intent = new Intent("ACTION_MSG_SENT");
        //sử dụng pending intent thông suốt ứng dụng Activity
        ...
        registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if(result == RESULT_OK) {
                    ShowLienHe.Notify(getApplicationContext(),"Đã gửi thành công!");
                    scroll_mess.setVisibility(View.GONE);
                    img_mess.setVisibility(View.VISIBLE);
                    text_mess.clearComposingText();
                }else{
                    ShowLienHe.Notify(getApplicationContext(),getString(R.string.mess_send_error));
                }
            }
